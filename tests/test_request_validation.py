...
class TestFormatPatientDetails(unittest.TestCase):
    ...
    def test_patient_line_format(self):
        lines = _format_patient_details(self._make_pd())
        self.assertEqual(
            lines[0],
            "Patient: Jane Smith, DOB 15 March 1990, Postcode SW1A 1AA"
        )
    ...
    def test_someone_else_with_relationship_produces_two_lines(self):
        ...
        self.assertEqual(lines[1], "Submitted by: John Doe (father)")

    def test_someone_else_without_relationship_omits_parentheses(self):
        pd = self._make_pd(
            patient_for="someone_else",
            submitter_name="John Doe",
            submitter_relationship=None,
        ...
        lines = _format_patient_details(pd)
        self.assertEqual(len(lines), 2)
        self.assertEqual(lines[1], "Submitted by: John Doe")

    def test_someone_else_without_submitter_name_produces_one_line(self):
        # submitter_name absent — submitted by line is suppressed
        pd = self._make_pd(
            patient_for="someone_else",
            submitter_name=None,
            submitter_relationship="mother",
