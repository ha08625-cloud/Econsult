...
# read from two places is one that can disagree with itself. Nothing in this
# module reads it any more -- pool membership and the sidecar tallies both go
# through the vector -- but importers do, and the re-export is the fact that
# there is one table.
...
#:
#: 4 is where a fragment's pool membership starts being computed from its label
#: vector rather than from ``(signal_key, fragment_type)``, and where the
#: declarative draw arrives. Both are inert at ``--declarative-share 0``: the
#: content digest is unchanged and a test asserts it. The version still moves,
#: because ``meta.generator_version`` is in every record and a dataset whose
#: pools were computed a different way is not comparable with one whose were
#: not, however identical this particular pair happens to be.
GENERATOR_VERSION = 4
...
#: Probability that a ``true`` or ``false`` example's **decisive** fragment is
#: drawn from the declarative pool rather than from the hand-written pool for
#: the same label. 0.0 skips the draw entirely -- no randomness is consumed and
#: the fragments chosen for a given seed are exactly the ones chosen before this
#: existed, which is what makes an arm at 0.0 a control rather than a second
#: treatment (DD8).
#:
#: The flag exists because the generated library is larger than any hand-written
#: one, so a uniform draw over a merged pool would make the generated frame the
#: *typical* decisive sentence and trade the one-claim prior for a one-frame
#: prior. This decouples how many lines were generated from how much of the
#: dataset they are.
#:
#: It governs the decisive slot only. A declarative fragment that is ``null`` on
#: the run's signal is a companion at any share, this one included: it was never
#: on the decisive path.
DEFAULT_DECLARATIVE_SHARE = 0.0
...
class FragmentPools:
    ...
    #: Declarative fragments asserting ``True`` for ``signal_key``, held apart
    #: from ``positive`` rather than merged into it. Merging would make the
    #: share of the decisive draw they win a function of how many lines were
    #: generated, which is exactly the coupling ``--declarative-share`` exists
    #: to break (DD8).
    declarative_positive: tuple[Fragment, ...] = ()
    #: As :attr:`declarative_positive`, for ``False``.
    declarative_negative: tuple[Fragment, ...] = ()
...
def asserted_signals(fragment: Fragment) -> tuple[str, ...]:
    """Every signal this fragment states a ``True`` or ``False`` for, sorted.

    One for a hand-written decisive fragment, none for an ambiguous one or for
    filler, and two to four for a declarative line. Sorted, because it seeds an
    exclusion list that a draw then iterates.
    """
    labels = fragment.labels or {}
    return tuple(sorted(signal for signal, value in labels.items() if value is not None))


def occupied_signals(fragment: Fragment) -> tuple[str, ...]:
    """The signals a fragment takes off the table for the rest of an example.

    Everything it asserts, plus its own signal if it has one. The second half is
    what keeps an *ambiguous* companion -- which asserts nothing but is very much
    about its signal -- from sitting beside that signal's positive fragment. For
    every hand-written fragment this is exactly the one-element
    ``(signal_key,)`` the companion draw excluded before vectors existed, which
    is why the pools and the draws come out unchanged.
    """
    signals = set(asserted_signals(fragment))
    if fragment.signal_key is not None:
        signals.add(fragment.signal_key)
...
def build_pools(fragments: Iterable[Fragment], signal_key: str, split: str) -> FragmentPools:
    ...
    Membership is decided by each fragment's **label vector** -- what it says
    about ``signal_key`` -- and not by its library's ``(signal_key,
    fragment_type)`` pair (DD4). For the 49 text libraries the two are the same
    fact: a text fragment's vector is derived from that pair plus ``null_on``,
    so every pool comes out byte-identical and the content digest holds. What
    changes is that a fragment whose polarity varies per *line* can now be
    placed at all.

    ==================================== ====================================
    the fragment's value for the signal  pool
    ==================================== ====================================
    ``True`` / ``False``                 decisive: ``positive`` / ``negative``,
                                         or their ``declarative_*`` twins
    ``None`` on its own signal           ``ambiguous``
    ``None`` on a foreign signal         ``companion``, under every signal it
                                         asserts
    undeclared                           ineligible
    ==================================== ====================================

    A fragment reaches a non-decisive slot only if it is *declared* ``null`` on
    ``signal_key`` -- for a text library that is the manifest's ``null_on``, for
    a declarative line it is a ``null`` in the line's own vector. Two
    consequences, unchanged from when the declaration arrived:
    ...
    * **Other signals' fragments are collected instead of dropped.** An
      *undeclared* pair is still ineligible, which is the honest state rather
      than a workaround: a smaller companion pool is a smaller dataset, not a
      wrong one.
    ...
    null, and ``--null-ambiguous-ratio`` would stop distinguishing anything. For
    a declarative fragment the same rule reads off the vector: it is a companion
    only where it is ``null`` on the run's signal, and it is then filed under
    *every* signal it does assert, because any of them is a reason to draw it and
    all of them are then spoken for (DD5).
    """
    in_split = sorted(
    ...
    positive: list[Fragment] = []
    negative: list[Fragment] = []
    ambiguous: list[Fragment] = []
    declarative_positive: list[Fragment] = []
    declarative_negative: list[Fragment] = []
    ...
    for fragment in in_split:
        value = fragment.value_for(signal_key)
        if fragment.fragment_type == "filler":
            # Filler-ness is a library property and stays one: a filler
            # fragment asserts nothing, so its only question is whether it has
            # been declared silent on this run's signal.
            if value is None:
        ...
        if value is UNDECLARED:
            continue
        if fragment.signal_key == signal_key:
            # A hand-written library for this run's own signal. Its vector says
            # True, False or None, which is positive, negative and
            # ambiguous/confounder respectively.
            (positive if value is True else negative if value is False else ambiguous).append(
                fragment
            )
        elif value is True:
            declarative_positive.append(fragment)
        elif value is False:
            declarative_negative.append(fragment)
        else:
            for signal in occupied_signals(fragment):
                companion_by_pair.setdefault((signal, fragment.library), []).append(fragment)
    ...
    pools = FragmentPools(
        ...
        declarative_positive=tuple(declarative_positive),
        declarative_negative=tuple(declarative_negative),
    )
    _check_pools(pools)
    return pools
...
def label_vector(
    ...
    **Label-first survives.** Nothing here reads a fragment's *text*. A
    fragment's contribution to every signal is fixed before it is drawn -- it is
    the vector its library derived or its line stated, read off
    :meth:`~.manifest.Fragment.value_for`. For a declarative line the direction
    is stronger still: the text was *constructed from* the vector, so there is
    nothing for a reader to judge.

    The candidate signals are the union of the fragments' vectors, which needs no
    ruleset: a signal no fragment names is one every fragment is undeclared on,
    so the table masks it anyway.
    ...
    for fragment in fragments:
        candidates.update(fragment.labels or {})
        for signal in asserted_signals(fragment):
            if signal in asserted:
                # DD5 makes this unreachable: the decisive slot is the primary
                # signal's only entry, and the companion draw excludes every
                # signal anything already chosen asserts. Raised rather than
                # resolved because two assertions are either redundant or
                # contradictory, and picking one of them silently is how a
                # dataset acquires a wrong label.
                raise AssertionError(
                    f"two fragments assert {signal!r} in one example: "
                    f"{source[signal]} and {fragment.fragment_id} "
                    f"(all: {sorted(f.fragment_id for f in fragments)})"
                )
            asserted[signal] = fragment.value_for(signal)
            source[signal] = fragment.fragment_id

    vector: dict[str, bool | None] = {}
    for signal in sorted(candidates):
        if all(f.value_for(signal) is not UNDECLARED for f in fragments):
...
def _draw_companion(
    rng: random.Random, pools: FragmentPools, exclude: Collection[str] = ()
) -> Fragment:
    ...
    ``exclude`` carries every signal anything already chosen *asserts or occupies*
    -- seeded from the decisive fragment, and widened by each companion drawn --
    so no example asserts one signal twice (DD5). Two assertions would either
    agree, doubling the evidence for one claim and teaching nothing, or disagree,
    which no single emitted label could describe. A declarative fragment
    asserting fever and dysuria therefore takes *both* out of the draw, not only
    the signal it happened to be filed under.
    ...
    # A declarative fragment is filed under every signal it asserts, so the pool
    # for one of those signals can hold a fragment that also asserts an excluded
    # one. Filtered here rather than by rejecting after the draw, because a
    # rejection loop would consume a variable number of draws and the stream
    # would stop being reproducible from the seed alone. For a hand-written
    # library the filter removes nothing: every fragment in the pool for signal
    # S occupies exactly S.
    candidates = [
        fragment
        for fragment in pools.companion_pool(signal, library)
        if not (set(occupied_signals(fragment)) - {signal}) & set(exclude)
    ]
    if not candidates:
        raise PoolError(
            f"split {pools.split!r} has no {signal!r} companion in {library!r} that avoids "
            f"the signals already asserted ({sorted(exclude)})"
        )
...
def select_fragments(
    ...
    *,
    ...
    declarative_share: float = DEFAULT_DECLARATIVE_SHARE,
) -> list[Fragment]:
    ...
    declarative_pools = {
        "true": pools.declarative_positive,
        "false": pools.declarative_negative,
    }
    if label_mode in signal_pools:
        # DD8: at share 0 the branch is skipped rather than taken with
        # probability zero. Taking it would consume a random() and move every
        # subsequent draw, and an arm that cannot be turned fully off cannot be
        # compared against its own absence.
        #
        # null_ambiguous is deliberately not in declarative_pools: every
        # generated line is an easy, unhedged, canonical claim, and a fixed
        # frame cannot express a hedge, a metaphor or a third-party attribution
        # (DD3). The hard-case libraries stay the only source of hard cases.
        pool = signal_pools[label_mode]
        # Short-circuit order is load-bearing: rng.random() is reached only
        # once the share is non-zero and the pool is non-empty.
        if (
            declarative_share > 0.0
            and declarative_pools.get(label_mode)
            and rng.random() < declarative_share
        ):
            pool = declarative_pools[label_mode]
    ...
    # One assertion per signal, tracked the way used_libraries tracks filler and
    # seeded from the decisive fragment (DD5). Seeding changes nothing for a
    # hand-written decisive fragment -- the primary signal is never in
    # pools.companion -- and is what stops a fever/dysuria declarative decisive
    # fragment from drawing a dysuria companion.
    used_signals: set[str] = {signal for f in chosen for signal in occupied_signals(f)}
    for _ in range(companions_wanted):
        try:
            companion = _draw_companion(rng, pools, exclude=used_signals)
        except PoolError:
            # Reachable only with declarative companions in the pool: each one
            # spends two to four signals on one slot, so the signals can run out
            # before the slots do. companion_bounds counts slots and cannot see
            # this coming, so the slot falls back to filler rather than failing
            # the run -- a slightly filler-heavier example, not a wrong one.
            filler = _draw_filler(rng, pools, exclude=used_libraries)
            used_libraries.append(filler.library)
            chosen.append(filler)
            continue
        used_signals.update(occupied_signals(companion))
...
def generate(
    ...
    *,
    ...
    if not 0.0 <= declarative_share <= 1.0:
        raise DistributionError(f"declarative-share must be in [0, 1]: {declarative_share}")
    ...
    if declarative_share > 0.0:
        # Loud rather than a silent fallback to the hand-written pool, for the
        # reason the companion check above is loud: a run asked for declarative
        # fragments and would otherwise produce the control arm's dataset under
        # the treatment arm's flags. Checked per label mode and only where the
        # distribution actually asks for that mode, because an empty declarative
        # pool is a normal state -- _check_pools deliberately does not raise on
        # one -- and a run at dist true=0 has no use for the positive pool.
        missing = [
            mode
            for mode, pool in (
                ("true", pools.declarative_positive),
            ...
            if not pool and distribution.get(mode, 0.0) > 0.0
        ]
        if missing:
            raise PoolError(
                f"--declarative-share {declarative_share} was requested but split "
                f"{pools.split!r} has no declarative fragment asserting "
                f"{' or '.join(missing)} for signal {pools.signal_key!r}"
            )
    ...
    for index in range(count):
        ...
        for attempt in range(MAX_ASSEMBLY_RETRIES + 1):
            fragments = select_fragments(
                ...
                spec.fragment_count,
                companion_share=companion_share,
                declarative_share=declarative_share,
            )
...
def _companion_stats(
    ...
    *which* companion rather than how many: if ``true`` examples drew ``true``
    ...
    stored per record: a fragment that occupies at least one signal, none of them
    the primary, reached a non-decisive slot, which is what a companion is. Read
    off the label vector rather than off ``signal_key``, because a declarative
    companion has no single signal and would otherwise be counted as filler --
    the tally that exists to detect a leak would then be blind to the fragments
    most able to cause one.

    A declarative companion is **one** count in ``count_by_*`` (it fills one
    slot) and contributes to ``signals`` and ``label_mix_by_label`` once per
    signal it asserts, because those two rows are about what the text says rather
    than about how many slots it took.
    """
    origin = {fragment.fragment_id: fragment for fragment in fragments}
    ...
    for example in examples:
        ...
        for fragment_id in example.meta["fragment_ids"]:
            fragment = origin.get(fragment_id)
            if fragment is None:
                continue
            occupied = occupied_signals(fragment)
            if not occupied or signal_key in occupied:
                continue
            drawn += 1
            for fragment_signal in occupied:
                signals[fragment_signal] = signals.get(fragment_signal, 0) + 1
                label_mix[label][_label_name(fragment.value_for(fragment_signal))] += 1
        count_key = str(drawn)
...
def _declarative_stats(
    ...
    *,
    ...
    """Tally the declarative draw: the DD7 leak detector, plus the realised mix.

    ``frame_by_label_mode`` is the row that matters, and it is the exact analogue
    of ``companions.count_by_label_mode``. The frames are label-neutral by
    construction -- each one generates every polarity, so the positive base
    produces all-true sentences *and* true-then-false mixed ones -- but "by
    construction" is a claim, and a claim about training data that nothing checks
    is a claim that quietly stops being true. If one frame skews towards one
    label mode, frame identity is a cue for the label, and the run is void rather
    than reinterpretable.

    A line with no ``meta.frame`` is counted under ``"unknown"`` rather than
    dropped: a generated library that forgot to record its frames should read as
    unchecked, not as checked and clean.

    ``count_by_label_mode`` is how many declarative fragments an example holds,
    decisive and companion alike. It is the answer to "how much of this dataset
    is generated text", which ``--declarative-share`` only *requests*.
    """
    origin = {fragment.fragment_id: fragment for fragment in fragments}
    frames = sorted(
        {
            str(fragment.meta.get("frame", "unknown"))
            for fragment in fragments
            if fragment.fragment_type == DECLARATIVE_TYPE
        }
    )
    count_keys = [str(n) for n in range(max_fragment_count + 1)]

    def _empty_counts() -> dict[str, int]:
        return {key: 0 for key in count_keys}

    def _empty_frames() -> dict[str, int]:
        return {frame: 0 for frame in frames}

    by_label = {label: _empty_counts() for label in LABELS}
    by_label_mode = {mode: _empty_counts() for mode in LABEL_MODES}
    frame_by_label_mode = {mode: _empty_frames() for mode in LABEL_MODES}
    decisive_by_label_mode = {mode: 0 for mode in LABEL_MODES}
    ...
    for example in examples:
        ...
        for fragment_id in example.meta["fragment_ids"]:
            fragment = origin.get(fragment_id)
            if fragment is None or fragment.fragment_type != DECLARATIVE_TYPE:
                continue
            drawn += 1
            frame = str(fragment.meta.get("frame", "unknown"))
            frame_by_label_mode[mode][frame] = frame_by_label_mode[mode].get(frame, 0) + 1
            # Decisive iff it asserts the run's own signal: every non-decisive
            # slot is filled from a pool filtered on null for that signal.
            if fragment.value_for(signal_key) is not None:
                decisive_by_label_mode[mode] += 1
        count_key = str(drawn)
        by_label[label][count_key] = by_label[label].get(count_key, 0) + 1
        by_label_mode[mode][count_key] = by_label_mode[mode].get(count_key, 0) + 1
    ...
    return {
        "count_by_label": by_label,
        "count_by_label_mode": by_label_mode,
        "frame_by_label_mode": frame_by_label_mode,
        "decisive_by_label_mode": decisive_by_label_mode,
    }
...
def build_stats(
    ...
    *,
    ...
    return {
        ...
        # The DD7 leak detector, read like the two blocks above it: the
        # by_label_mode rows must agree with each other. See _declarative_stats.
        "declarative": _declarative_stats(
            examples,
            fragments=fragments,
            signal_key=signal_key,
            max_fragment_count=max(fragment_counts),
        ...
        "split_pool_sizes": {
            ...
            "declarative_positive": len(pools.declarative_positive),
            "declarative_negative": len(pools.declarative_negative),
        },
