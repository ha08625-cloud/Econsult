...
submissions, which every arm scores identically.
    ...
    python -m scripts.encoder_training declarative-compare --folds 5 \
        --cell .../c0.0-d0.0 --cell .../c0.5-d0.3

The procedural multi-symptom sweep, stem ``<signal>.declarative_comparison``. Two
to five *cells*, a cell being one ``(--companion-share, --declarative-share)``
pair, each fine-tuned against its own tree and put in one report per signal.
Unlike ``companion-compare`` these are per-signal trees rather than merged ones,
and cells may share a companion share -- which is the whole point, since the
question is whether declarative fragments add anything *on top of* companions
rather than on top of nothing. Both shares are read back from each cell's own
sidecars and asserted distinct. No cell pairs with any other on the synthetic
test set, so every McNemar row is a recorded skip and the 67 real-text
submissions are the shared instrument.
...
# ---------------------------------------------------------------------------
# The declarative comparison (procedural multi-symptom fragments, task 7)
# ---------------------------------------------------------------------------

#: What the run is expected to show, recorded in every report it writes so that
#: a result can disagree with it. Lifted from the implementation plan's
#: "Predictions, recorded before the run"; the ones this command can actually
#: decide, in the words they were written in.
DECLARATIVE_PREDICTIONS = (
    "The invented-symptom rate on real text improves, most for the signals with the most "
    "inventory phrases and least for flank_pain_present.",
    "`false` recall improves most: explicit denials are 13% of the real set and 'but not X' is "
    "the construction patients use for them.",
    "A cell at declarative 0.6 scores worse on real text than one at 0.3, because the frame "
    "becomes the typical decisive sentence. If 0.6 wins, DD8's argument is wrong.",
    "Against a companion cell rather than against the all-zero control, the gain is smaller than "
    "it looks: companions already move null -> true from 84.1% to 4.5% on fever_present by "
    "themselves.",
)


def _cell_shares(directory: Path, signals: Sequence[str], folds: int) -> dict:
    """The two shares a cell's datasets were *written* with, from their sidecars.

    Read from the sidecars rather than from this command's flags, for the reason
    :func:`_companion_facts` gives: the cell was generated by an earlier
    invocation and what belongs beside a number is a description of the data the
    number came from, not of the run that read it.

    Both shares degrade to ``None`` rather than raising, and ``None`` is what
    :func:`run_declarative_compare`'s distinctness check refuses on -- a cell
    whose settings cannot be read cannot be shown to differ from any other.

    Takes a fold *count* rather than loaded folds, so that every cell's
    coordinates can be read and checked before a single tree is loaded. Two
    cells at the same shares is the mistake this catches, it is made with a
    dropdown rather than with a path, and catching it after four trees are in
    memory would be catching it no sooner in any way that matters.
    """
    companion: object = None
    declarative: object = None
    version: object = None
    splits_read = 0
    for signal in signals:
        for fold_index in range(folds):
            path = directory / f"{signal}.fold{fold_index}.train.jsonl.stats.json"
            if not path.exists():
                continue
            stats = json.loads(path.read_text(encoding="utf-8"))
            requested = stats.get("requested") or {}
            companion = requested.get("companion_share")
            declarative = requested.get("declarative_share")
            version = stats.get("generator_version")
            splits_read += 1
    return {
        "dataset_dir": str(directory),
...
def _cell_label(shares: Mapping[str, object]) -> str:
    """``c0.5-d0.3`` -- the cell's coordinates, not its directory name.

    Derived from the sidecars so that a directory named after the wrong cell
    cannot mislabel a column. It is also what the console's entries name their
    directories, so the two agree without either reading the other.
    """
    return f"c{shares['companion_share']}-d{shares['declarative_share']}"


def run_declarative_compare(args: argparse.Namespace) -> int:
    """Two to five cells of the declarative sweep, in one report per signal.

    ``companion-compare`` is the near neighbour and cannot do this: it wants
    merged ``joint6`` trees, and it refuses two trees that share a
    ``--companion-share`` -- which cells A and B do, and C and D do. So this is
    ``joint-compare``'s shape instead, minus the joint arm: N single-signal arms,
    each fine-tuned against its own cell, restricted to one signal and put in
    that signal's report together.

    **No two cells are paired on the synthetic test set, including the first
    two.** Changing either share changes which fragments are drawn and therefore
    what the texts are, while the example ids are still numbered from zero in
    every cell -- so the id sets match exactly while the texts behind them do
    not. Every cell after the reference is qualified by
    :func:`_as_unpaired`, which turns its McNemar rows into recorded skips
    rather than a paired test over pairs that do not exist. The 67 real-text
    submissions are the same for every cell, and that is where the question is
    decided.

    **The first ``--cell`` is the reference.** Its tree is what the report's test
    slice, fold partition and cluster checks describe, for the reason
    ``_emit_joint_reports`` gives about A1: pooling several trees' test clusters
    into one partition statement would make a true sentence about each into a
    false one about all of them.
    """
    signals = tuple(args.signals)
    cells = [Path(directory) for directory in args.cell]
    if len(cells) < 2:
        raise TrainError(
            f"a declarative comparison needs at least two cells, got {[str(c) for c in cells]}. "
            "Pass --cell once per cell, reference first"
        )
    duplicates = sorted({str(cell) for cell in cells if cells.count(cell) > 1})
    if duplicates:
        raise TrainError(
            f"--cell repeats a directory: {', '.join(duplicates)}. One directory is one cell, so "
            "this would train the same arm twice and report the difference between two seeds as "
            "the effect of a share"
        )

    # The coordinates first, from the sidecars alone. A cell repeated under two
    # names is the mistake a dropdown makes, and it is worth refusing before the
    # trees are read, let alone before a weight is downloaded.
    shares_by_cell = {cell: _cell_shares(cell, signals, args.folds) for cell in cells}

    missing = [str(cell) for cell in cells if not cell.is_dir()]
    if missing:
        raise TrainError(
            f"no such directory: {', '.join(missing)}. Generate the cell first -- this command "
            "reads fold trees, it does not build them"
        )

    unreadable = [
        str(cell)
        for cell in cells
        if shares_by_cell[cell]["companion_share"] is None
        or shares_by_cell[cell]["declarative_share"] is None
    ]
    if unreadable:
        raise TrainError(
            f"no companion or declarative share in the sidecars under: {', '.join(unreadable)}. "
            "These are pre-version-4 trees, or trees generated before generate-folds forwarded "
            "the shares; regenerate them rather than assuming what they were built with"
        )

    seen: dict[tuple[object, object], Path] = {}
    for cell in cells:
        shares = shares_by_cell[cell]
        key = (shares["companion_share"], shares["declarative_share"])
        if key in seen:
            raise TrainError(
                f"{cell} and {seen[key]} were both generated at --companion-share {key[0]} and "
                f"--declarative-share {key[1]}. These are two runs of one cell, not two cells, "
                "and the comparison between them would be a comparison of seeds"
            )
        seen[key] = cell

    if not any(shares_by_cell[cell]["declarative_share"] for cell in cells):
        raise TrainError(
            "every cell was generated at --declarative-share 0, so no cell holds a declarative "
            "fragment and there is nothing here to compare. This is the run that would report a "
            "null result most convincingly and mean nothing by it"
        )

    # Every tree, and the holdout, before a single weight is downloaded: a bad
    # path must cost a second rather than hours of GPU followed by a traceback.
    folds_by_cell = {
        cell: {signal: load_folds(cell, signal, folds=args.folds) for signal in signals}
        for cell in cells
    }
    holdout = _load_holdout(args)
    cuda_required = not args.allow_cpu and args.device != "cpu"
    ...
    reference_encoder = factory()
    _warn_if_unpinned(reference_encoder)
    encoder_facts = reference_encoder.to_dict()
    encoder_header = {
        "model_revision": reference_encoder.revision,
        "revision_pinned": reference_encoder.revision_pinned,
        "tokeniser_lowercases": reference_encoder.facts.lowercases_input,
        "tokeniser_vocab_size": reference_encoder.facts.vocab_size,
        "tokeniser_discards_casing": reference_encoder.facts.discards_casing,
    }
    ...
    arms = [
        _run_single_signal_arm(
            args,
            label=_cell_label(shares_by_cell[cell]),
            directory=cell,
            folds_by_signal=folds_by_cell[cell],
            factory=factory,
            config=config,
            encoder_facts=encoder_facts,
            device_facts=device_facts,
    ...
    return _emit_declarative_reports(
        args,
        arms=arms,
        cells=cells,
        signals=signals,
        shares_by_cell=shares_by_cell,
...
def _emit_declarative_reports(
    args: argparse.Namespace,
    *,
    arms: Sequence[JointArm],
    cells: Sequence[Path],
    signals: Sequence[str],
    shares_by_cell: Mapping[Path, Mapping[str, object]],
    folds_by_cell: Mapping[Path, Mapping[str, Sequence]],
    device: str,
    holdout,
    encoder_header: Mapping[str, object],
) -> int:
    """One report per signal, each holding every cell restricted to that signal."""
    boot = BootstrapConfig(resamples=args.resamples, seed=args.bootstrap_seed, alpha=args.alpha)
    reference = cells[0]
    reference_folds = folds_by_cell[reference]

    # The union across cells, deduplicated by fragment id, and filtered per
    # report to its own signal: a coverage block missing a cell's libraries would
    # leave the interval-width warning unprinted for the one arm it is about.
    fragments = _fragment_provenance(
        [fold for arm in arms for folds in arm.folds_by_signal.values() for fold in folds]
    )
    ...
    declarative = {
        "note": (
            "One cell per `--cell`, each generated from the same libraries with the same seed, "
            "the same counts and the same fold triple, differing only in `--companion-share` and "
            "`--declarative-share`. The shares are read from each cell's own sidecars, not from "
            "this run's flags."
        ),
        "cells": [dict(shares_by_cell[cell]) for cell in cells],
        "reference": (
            f"`{_cell_label(shares_by_cell[reference])}` ({reference}) is the reference: the "
            "report's test slice, fold partition and cluster checks describe its tree."
        ),
        "pairing": (
            "No two cells are paired on the synthetic test set. Changing either share changes "
            "which fragments are drawn, so the example ids match across cells while the texts "
            "behind them do not; every cell after the reference is recorded as unpaired and its "
            "McNemar rows are skips rather than a test over pairs that do not exist. The 67 "
            "real-text submissions are the same for every cell and are where the cells are "
            "compared."
        ),
        "predictions": list(DECLARATIVE_PREDICTIONS),
    }
    ...
    for signal in signals:
        ...
        if not args.no_baselines:
            runs.extend(
                run_all(reference_folds[signal], signal=signal, shuffle_seed=args.shuffle_seed)
            )
        runs.extend(arm.runs[signal] for arm in arms)
        runs.extend(arm.control_runs[signal] for arm in arms if arm.control_runs is not None)

        per_signal_args = argparse.Namespace(
            **{**vars(args), "signal": signal, "data_dir": reference}
        )
        header = _header(
            per_signal_args,
            reference_folds[signal],
            extra={
                "report": f"declarative sweep, {len(cells)} cells",
                "arms": [
                    f"**{arm.label}** (`{shares_by_cell[cell]['dataset_dir']}`): "
                    f"{len(arm.folds_by_signal[signal][0].train)} examples per epoch, "
                    f"**{_labelled_positions(arm.folds_by_signal[signal], signal)}** labelled "
                    f"positions for `{signal}`"
                    for arm, cell in zip(arms, cells, strict=True)
                ],
                "cells": [_cell_label(shares_by_cell[cell]) for cell in cells],
                "reference_cell": str(reference),
                "paired_comparison": (
                    "none. Every cell holds different texts, so all McNemar rows are recorded as "
                    "skipped; the real-text holdout is the shared instrument"
                ),
                "selected_epochs": {
                    arm.label: _selected_epochs(arm.results[signal], signal) for arm in arms
                },
                ...
                "validation_guided_decisions": list(VALIDATION_GUIDED_DECISIONS),
                "negative_control": (
                    "run per cell" if args.control else "not run (--control is off)"
                ),
                "artefacts": str(args.models_dir),
                **encoder_header,
            },
        ...
        report = build_report(
            ...
            boot=boot,
            checks={
                **_checks(reference_folds[signal]),
                "cell_datasets": (
                    "checked at load. Every cell's tree was loaded, and its two shares read from "
                    "its own sidecars and asserted distinct, before any training started. The "
                    "checks above describe the reference cell's tree, which is the one this "
                    "report's test slice belongs to."
                ),
            },
            fragments=fragments,
        ...
        json_path, markdown_path = write_report(
            report,
            args.report_dir,
            stem=f"{signal}.declarative_comparison",
            markdown=not args.no_markdown,
            fragment_rows=args.fragment_rows,
        )
        print(f"wrote {json_path}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    declarative = subparsers.add_parser(
        "declarative-compare",
        help="two to five cells of the declarative sweep in one report per signal: the "
        "procedural multi-symptom comparison",
    )
    _add_report_args(declarative)
    _add_encoder_args(declarative)
    _add_holdout_args(declarative)
    declarative.add_argument(
        "--signals",
        nargs="+",
        default=list(DEFAULT_SIGNALS),
        help="the heads to train and compare, one report each. Defaults to the six with fragment "
        "libraries",
    )
    declarative.add_argument(
        "--cell",
        ...
        required=True,
        metavar="DIR",
        help="one cell's fold tree, repeated once per cell, two to five of them. A cell is a "
        "(--companion-share, --declarative-share) pair; the two are read back from each tree's "
        "own sidecars and asserted distinct, so a directory named after the wrong cell cannot "
        "mislabel a column. THE FIRST IS THE REFERENCE: its tree is what the report's test "
        "slice, fold partition and cluster checks describe. Unlike companion-compare these are "
        "per-signal trees, not merged ones, so no merge-folds step is needed",
    )
    declarative.add_argument("--models-dir", type=Path, default=DEFAULT_MODELS_DIR)
    declarative.add_argument("--cache-dir", type=Path, default=DEFAULT_CACHE_DIR)
    declarative.add_argument("--device", default="auto")
    declarative.add_argument("--epochs", type=int, default=FineTuneConfig.epochs)
    declarative.add_argument("--batch-size", type=int, default=FineTuneConfig.batch_size)
    declarative.add_argument("--lr", type=float, default=FineTuneConfig.lr)
    declarative.add_argument("--weight-decay", type=float, default=FineTuneConfig.weight_decay)
    declarative.add_argument("--warmup-ratio", type=float, default=FineTuneConfig.warmup_ratio)
    declarative.add_argument("--max-grad-norm", type=float, default=FineTuneConfig.max_grad_norm)
    declarative.add_argument("--eval-batch-size", type=int, default=FineTuneConfig.eval_batch_size)
    declarative.add_argument(
        "--determinism", choices=DETERMINISM_MODES, default=FineTuneConfig.determinism
    )
    declarative.add_argument("--train-seed", type=int, default=FineTuneConfig.seed)
    declarative.add_argument("--allow-cpu", action="store_true")
    declarative.add_argument(
        "--no-weights",
        action="store_true",
        help="do not write the fine-tuned encoders. Four cells of six signals of five folds at "
        "~440MB is ~53GB; none of it is committed and all of it is regenerable from the pinned "
        "seeds",
    )
    declarative.add_argument(
        "--control",
        action="store_true",
        help="run the shuffled-label negative control for every cell. Off by default, as in "
        "joint-compare: it would double the sweep and it answers a question the single-signal "
        "runs on these same datasets already answered",
    )
    declarative.add_argument(
        "--no-baselines",
        action="store_true",
        help="omit the baselines. They are fitted on the reference cell's folds -- the tree this "
        "report's test slice belongs to -- once per signal",
    )
    declarative.add_argument(
        "--progress", action="store_true", help="print per-epoch training loss and validation score"
    )
    declarative.set_defaults(handler=run_declarative_compare)
