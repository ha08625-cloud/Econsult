...
The SlowAPI `Limiter` instance in `app/core/rate_limit.py` is module-level and uses in-memory storage. Its counters persist across test boundaries within a single pytest session. Any test class that fires multiple requests to a rate-limited endpoint (such as `TestAuthRateLimiting`) would contaminate later tests that simulate normal single-request traffic, causing spurious 429 failures. The `autouse=True` fixture in `conftest.py` resets the storage before and after every test, making each test independent of request history from previous tests.

### Why does TestAuthRateLimiting use with_rate_limiting=True rather than the default make_test_app?
...
### Why are auth_service functions patched in TestAuthRateLimiting?
`StubAuthRepo.get_user_by_email` and `StubAuthRepo.get_auth_code_record` both return `None` unconditionally, so an unpatched call to `verify_login_credentials` or `verify_mfa_code` fails validation on every request (raising `INVALID_CREDENTIALS` / `INVALID_AUTH_CODE`, HTTP 422) rather than succeeding. Patching these functions lets the first five requests complete as if the login/OTP check had succeeded, so the test demonstrates that slowapi's IP-based limiter blocks the sixth request regardless of business-logic outcome. There is no service-layer rate limit competing with slowapi here: `auth_service.request_mfa_code` defines a per-email cooldown but is currently dead code — `admin_auth_router.login` generates and upserts the OTP inline and never calls it. The `/auth/login` and `/auth/verify` endpoints therefore have no rate limiting of their own; slowapi's 5/minute limit is the only gate this test is exercising.
