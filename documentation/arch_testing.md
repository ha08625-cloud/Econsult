...
| Job | Gated on | Runs |
|---|---|---|
| `ruleset-validation` | `data/**` changed | `pytest tests/test_data_rulesets.py`, `pytest tests/test_synthetic_recombination.py` and `pytest tests/test_encoder_training_holdout.py` only — no database, no ruff, no frontend |
...
| `test_encoder_training_holdout.py` | `scripts/encoder_training/holdout.py`, the ordering in `train.select_then_score`, `report.holdout_summary`, and the committed `data/realistic/` files | The realistic held-out set. **No marker, no torch, no GPU** — which is the whole point of `holdout.py` taking its forward pass as an injected callable, so every line that decides what a real-text number *means* runs in CI. Three tests carry the load. `test_a_blank_cell_is_not_a_null` is README rule 4: a signal the labeller could not judge has its key omitted and is excluded from numerator and denominator alike, where `null` is a claim the model is scored on; merging them invents a label and would be invisible downstream. `test_the_holdout_is_scored_after_the_margin_and_after_test` is README rule 2, asserted with a recording fake over `select_then_score` rather than by reading `run_finetune_fold` — the set can select nothing because by the time it is opened the margin is fixed and test is already scored. `test_the_resampling_unit_is_the_submission` is rule 5: one submission is one observation whatever it labels, and treating its six cells as six would report an interval about sqrt(6) too narrow on the one set whose entire limitation is its size. Also: header validation against the ruleset's `send_to_encoder` signals, the majority-class trap the `decisive` slice exists to spring, per-fold aggregation refusing to pool five models over the same 67 submissions, and that the report prints both `n`s side by side. **Three of them read the committed data**: every submission id still lines up with its line in the source file (a shifted id attaches every label to the wrong text and nothing downstream could tell), the README's distribution table is recounted from the TSV, and no cell is blank. Those three are why this file also runs in the `ruleset-validation` CI job — a `data/`-only PR skips the `unit` job, so the guards would otherwise never run on the edit they were written for. |
| `test_encoder_training_merge.py` | `scripts/encoder_training/merge.py` plus the CLI's `merge-folds` surface | Merging the six per-signal fold trees into one multi-head tree. **No marker, no torch** — the merge decides what the joint run's numbers mean, so CI covers every line of it. The contract test is `test_load_folds_accepts_the_merged_tree`: the merged output must satisfy `_check_disjoint`, `_check_fold_agreement` and `_check_test_partition` unchanged, and if it ever needed a new escape hatch in `dataset.py` the merge would be wrong, not the loader. Three more carry the load. `test_merged_tree_keeps_every_head_masked_where_it_had_no_label` is the whole design: a dysuria example carries no `fever_present` key, which is a mask rather than a `null` assertion, so no head gains supervision from the merge — writing `null` there instead would teach every head to answer "not mentioned" on every other head's data and would be invisible until the model was mysteriously bad. `test_ids_are_namespaced_and_source_ids_restore_the_originals` is what makes the joint-vs-single comparison pairable: six trees all number from `train-000000`, so the merged id is namespaced and `meta.source_ids` carries the original back. And the structural-null tests assert the byte-identity the union rests on rather than trusting it — a signal term entering the generator's seed derivation would otherwise collapse six divergent null sets into whichever arrived first and shift every head's class prior silently. Also the failure paths (divergent nulls, conflicting fragment entries, a disagreeing salt or generator version, a tree missing a fold, an already-merged source) and that a cross-library cluster-key collision is *reported* rather than fatal, since it deflates effective n rather than inflating it. Fixtures are a miniature generator built in the test module, not committed files: a six-signal five-fold tree is forty-five files and the properties under test are only legible when the generator's rules sit beside the assertions. |
...
| `test_training_gui.py` | `scripts/training_gui/` (the local run console) | The catalogue, the runner, the two git sequences and the HTTP boundary. **No marker, no torch, no GPU, no database** — the runner tests start real subprocesses, but stdlib ones (`-m json.tool`, `-m timeit`) against `tmp_path`, so the execution path runs on an ordinary CI runner. Four groups carry the load. *The catalogue as a closed set*: a step must be a module invocation (`-m ...`), which mechanically rejects `python`, an absolute interpreter path and `git`; a parameter must enumerate its `choices` with a default among them; a placeholder no parameter declares, and a parameter no step uses, are both errors — a catalogue that reads plausibly and substitutes nothing is the failure mode. *The boundary*: an unknown id is a 404 and a value outside the committed `choices` a 400, both asserted to never reach the runner, and `test_no_endpoint_takes_a_path_or_a_branch_name_from_the_browser` pins that guarantee as a property of the API rather than of the loader. *Aborting and recording*: a failing first step leaves the second unrun (continuing produces a report tree that loads cleanly and means nothing), and the manifest records the commit the run was produced by. *The git sequences against a fake git*: the fixed order, branching from the manifest's sha rather than a remote branch, staging only `reports/` and `models/` and never `-a`, a failed fetch stopping before anything mutating, push last, a dirty path outside the two prefixes blocking the whole sequence, and a HEAD that moved since the run blocking it too. Also `test_fastapi_and_uvicorn_pins_match`, which is why AD1's duplicated pins cannot drift unnoticed. |
