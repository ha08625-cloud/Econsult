...
- **Handler concurrency rule.** All database access is blocking `psycopg2` (see `docs/arch_infrastructure.md`, Database). An `async def` handler runs its body directly on the single event loop thread, so any blocking call inside it stalls every other concurrent request in the process. A plain `def` handler is dispatched by FastAPI to the anyio threadpool instead, which is where every blocking database, bcrypt, or network call must run. The rule is now uniform: **every handler in `app/routers/` is a plain `def`.** `require_admin` (`app/core/admin_context.py`) is a plain `def` dependency for the same reason — it runs `get_session_context` and `update_session_expiry` on every authenticated request.

  A plain `def` handler cannot `await request.json()` / `.body()` / `.form()` to read its own body. Body-reading handlers instead call `read_json_body(request)` (`app/core/body_capture.py`), which returns the body captured by `BodyCapturingRoute` — an `APIRoute` subclass that reads the raw body once, in its own async wrapper, before the handler runs, and stashes it on `request.state.raw_body`. The class and the accessor are a pair: a router only gets `raw_body` if it is constructed with `APIRouter(route_class=BodyCapturingRoute)`, and `read_json_body` raises a named `BodyNotCaptured` error (not a bare `AttributeError`) if that wiring is missing. `read_json_body` also accepts `parse_float` for callers (e.g. `form_router.py`'s `form_update`) that need `Decimal` rather than `float` for numeric fields.

  **Three exemptions, each a decision rather than an oversight:**
  - `form_router.py`'s `form_finish` — no `route_class`. It already reads its body via `Form(...)`/`File(...)` parameters, which work in a plain `def` without `BodyCapturingRoute`; adding the route class would additionally buffer the raw body alongside the parsed multipart form, roughly doubling peak memory on a large photo upload.
  - `webhook_router.py`'s `mailgun_webhook` — stays `async def` as a thin shim (`await request.form()` plus a few `app.state` accessor reads), then dispatches the parsed payload to a plain `def _mailgun_webhook_impl` via `run_in_threadpool`. Mailgun's field names are dynamic (`event-data[message][headers][message-id]`), which neither `BodyCapturingRoute` nor a declared `Form(...)` parameter can supply.
