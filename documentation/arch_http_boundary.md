...
- **Handler concurrency rule.** All database access is blocking `psycopg2` (see `docs/arch_infrastructure.md`, Database). An `async def` handler runs its body directly on the single event loop thread, so any blocking call inside it stalls every other concurrent request in the process. A plain `def` handler is dispatched by FastAPI to the anyio threadpool instead. Therefore: a handler that does not read the request body (no `await request.json()` / `.body()` / `.form()`, no `UploadFile.read()`) must be a plain `def`. A handler that must stay `async def` because it reads the body is required to delegate its blocking work to the threadpool explicitly via `run_in_threadpool` (see `form_router.py`'s photo sanitisation and `admin_auth_router.py`'s `auth_service` calls). `require_admin` (`app/core/admin_context.py`) is a plain `def` dependency for the same reason — it runs `get_session_context` and `update_session_expiry` on every authenticated request. The sole exemption is `main.py`'s `/healthz`, which is `async def` but touches no database.
