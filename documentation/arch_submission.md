...
Finalizing forms, persisting submission records, auditing, photo storage, PDF generation, attachment storage, and delivering clinical output to the practice via a three-stage async pipeline (email path). The NHS MESH transport path is documented separately in `docs/arch_mesh.md`.

**Key files:** `serialisation.py`, `serialisation_contracts.py`, `submission_repository.py`, `attachment_repository.py`, `photo_repository.py`, `delivery_service.py`, `delivery_constants.py`, `pdf_formatter.py`, `pdf_repository.py`, `pdf_constants.py`, `delivery_repository.py`, `downstream_enqueuer.py`, `pdf_worker.py`, `pdf_worker_main.py`, `delivery_worker.py`, `worker_main.py`, `deletion_job.py`, `webhook_router.py`, `app/core/email_mode.py`
...
The MESH dispatch path (NHS transport alternative to the Mailgun email path below), its fallback-to-email ordering invariant, and its metadata access deviation are documented in `docs/arch_mesh.md`. The PDF worker's `downstream.enqueue` step (see "Ordering Invariant" above) is the boundary between this document and that one — everything before it is shared between the email and MESH paths; everything from `mesh_jobs` creation onward is MESH-specific and lives in `arch_mesh.md`.
...
MESH-specific. See `docs/arch_mesh.md`, "`mesh_jobs` Table."
...
MESH-specific. See `docs/arch_mesh.md`, "Retry Schedule."
...
**MESH path (Phase 3 interim):** pure-MESH submissions are never deletion-eligible until Phase 4 ships — this is a production enablement gate, not a cosmetic limitation. See `docs/arch_mesh.md`, "Data Retention — Interim Limitation," for the detail and the rationale.
...
- **`ClinicalOutput`** — lossy by design. Strips provenance and encoder internals. Safe for clinical and patient use. Contains `question_labels` (answer_key -> question text at submission time) so the record is self-contained and interpretable without reloading the ruleset. It also carries `pdf_labels` (answer_key -> short display label), snapshotted from the ruleset's `pdf_label` fields for the same reason as `question_labels` — the PDF worker regenerates documents from persisted JSONB and has no ruleset. Like `quantity_answers`, it defaults to an empty dict and is read via `.get()` in `from_dict`, so records predating it deserialise cleanly and simply render no summary. Provides a `from_dict` classmethod for reconstructing from the JSONB dict returned by psycopg2, which handles the nested `PatientDetails` dataclass. `ClinicalOutput` carries no top-level `unit_system` field. For quantity (unit-toggle) answers, the chosen system lives per answer_key in the `quantity_answers` sidecar, alongside `quantity_kind`, the lossless `raw_components`, and a `decimal_places` snapshot — four keys per sidecar entry. `answers[key]` still holds the canonical-unit string for those keys; the sidecar exists so the PDF (which has no ruleset) can render the patient's original unit and dispatch on kind without reloading the ruleset. `quantity_answers` defaults to an empty dict and is read via `.get()` in `from_dict`, so records predating quantity support deserialise cleanly — the same pattern as `photo_quality_tier`. A record persisted before the per-answer system change may still carry a stray top-level `unit_system` key in its stored JSONB; `from_dict` simply does not read it, since `.get()` on an unlisted key is a no-op.
...
- **Quantity answers, wire shape.** For a quantity question the client view carries `quantity`/`quantity_kind`/`allowed_systems`/`default_system` and overrides `current_value` to `{system, components}` (or null when unanswered). Components are emitted as **strings**, matching the existing scalar-Number convention where a number's `current_value` travels to the client as its canonical string. The client holds them verbatim while editing and converts to JSON numbers only when building the `/form/update` payload — so the wire is asymmetric by design: string components outbound, number components inbound. The PDF (`pdf_formatter.py`) dispatches on `quantity_kind` from the `quantity_answers` sidecar (defaulting to `"weight"` when the sidecar entry predates `quantity_kind`, per the `.get()` convention above) to select a per-kind formatter. The weight formatter renders imperial as `"11 st 11 lb (74.8 kg)"` and metric as the typed kilograms; the parenthetical kg (rounded to `decimal_places`) appears only for imperial, since metric raw input already is the canonical unit. The formatter derives metric-vs-imperial from the component keys present (`{kg}` vs `{st, lb}`), not from the stored `unit_system`, so it has no second code path for records that predate per-answer `unit_system`.
...
Service selection is single-sourced in `app/core/email_mode.py` (`select_email_delivery_mode`), consumed by both entry points that construct these classes: `main.py` (via `wiring.py`'s `build_container`) reaches it through `settings.email.delivery_mode` — see `docs/arch_http_boundary.md`, "Delivery Service Instantiation," for the predicate as exposed there — and `worker_main.py` calls it directly with raw `os.environ` values, since the worker has no settings object. Both require a complete Mailgun pair (`MAILGUN_API_KEY` + `MAILGUN_DOMAIN`) before selecting Mailgun; a partial configuration (one set, not both) logs a warning and falls through to SMTP on both paths, identically. This replaced an earlier divergence where `worker_main.py` selected on `MAILGUN_API_KEY` alone and crash-looped at `MailgunHttpDeliveryService.__init__` on a partial configuration that the web path already handled correctly.
