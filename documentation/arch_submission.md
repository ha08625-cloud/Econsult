...
- **`ClinicalOutput`** — lossy by design. Strips provenance and encoder internals. Safe for clinical and patient use. Contains `question_labels` (answer_key -> question text at submission time) so the record is self-contained and interpretable without reloading the ruleset. Provides a `from_dict` classmethod for reconstructing from the JSONB dict returned by psycopg2, which handles the nested `PatientDetails` dataclass. For quantity (unit-toggle) answers it also carries `unit_system` (the patient's chosen system) and a `quantity_answers` sidecar keyed by answer_key, each holding the lossless `raw_components` and a `decimal_places` snapshot. `answers[key]` still holds the canonical kg string for those keys; the sidecar exists so the PDF (which has no ruleset) can render the patient's original unit. Both new fields default (None / empty) and are read via `.get()` in `from_dict`, so records predating quantity support deserialise cleanly — the same pattern as `photo_quality_tier`.
...
- **Quantity answers, wire shape.** For a quantity question the client view carries `quantity`/`allowed_systems`/`default_system` and overrides `current_value` to `{system, components}` (or null when unanswered). Components are emitted as **strings**, matching the existing scalar-Number convention where a number's `current_value` travels to the client as its canonical string. The client holds them verbatim while editing and converts to JSON numbers only when building the `/form/update` payload — so the wire is asymmetric by design: string components outbound, number components inbound. The PDF (`pdf_formatter.py`) formats these from `quantity_answers`: imperial as `"11 st 11 lb (74.8 kg)"`, metric as the typed kilograms; the parenthetical kg (rounded to `decimal_places`) appears only for imperial, since metric raw input already is the canonical unit.
