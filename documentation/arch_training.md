...
the declaration pass decided. `build_pools` now filters filler by the declaration
and collects other signals' declared fragments into a companion pool; no draw
uses that pool yet, so every generated byte is still what it was.
...
* **Other signals' fragments are collected instead of dropped.** They used to be
  ...
  **Nothing draws from this pool yet** — the companion draw is the next piece —
  and a test asserts that no companion fragment reaches any generated example.
