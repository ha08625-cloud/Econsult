...
- **Every connection carries four timeouts** (constants in `db.py`, overridable per call, no environment variables): `connect_timeout` 5s, `statement_timeout` 10s, `idle_in_transaction_session_timeout` 30s, and TCP keepalives (30/10/3) plus `tcp_user_timeout` 30s. Each covers a different failure and none substitutes for another. In particular `statement_timeout` is enforced by the *server*, so it cannot fire when the server is what disappeared — keepalives are the only client-side bound on that case. That case matters most for the handlers that remain `async def` and call synchronous psycopg2 directly on the event loop thread with a single uvicorn worker: one connection blocked in `recv()` there stalls every concurrent request in the process, not just its own. See `docs/arch_http_boundary.md`'s handler-concurrency rule for which handlers this applies to and why the rest are plain `def`.
