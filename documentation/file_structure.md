...
- `data/` — Condition ruleset JSON files.
  - `realistic/` — The 67 hand-written realistic held-out submissions, their labels (`uti1_holdout.labels.tsv`, one column per `send_to_encoder` signal) and the arbitration notes for the arguable cells. Permanently held out: never trained on, never used to select anything. Its `README.md` is the authority on the rules; `arch_encoder_training.md` section 11 is what the tooling does with them. Not loaded by the application at any point.
...
- `scripts/` — Offline tooling: one-time management commands, the synthetic dataset generator, the encoder-training tooling, and the local training run console. Nothing here is imported by `app/`.
  ...
  - `encoder_training/` — Trains and evaluates encoder heads against those datasets. `generate-folds` writes all K folds' three splits; `merge-folds` concatenates the six per-signal fold trees into one multi-head tree that `load_folds` reads unchanged (see `arch_training.md` section 7); `baselines` fits the baselines and negative controls; `smoke-cuda` runs a real kernel on the training device (no model, no network — run it first on a new machine); `smoke` adds the encoder download; `probe` is Arm A, the frozen probe; `finetune` is Arm B, every layer unfrozen, and it also scores the real-text holdout in `data/realistic/` after each fold's margin and test split (`--no-holdout` skips it). Everything except the model-fitting bodies is stdlib only, so CI covers it with no ML wheels; the arms and `baselines` need `requirements-ml.txt`. Tested by `tests/test_encoder_training_{dataset,metrics,baselines,arm_a,arm_b,holdout,merge}.py`.
