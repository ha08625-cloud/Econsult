...
- `scripts/` — Offline tooling: one-time management commands, plus the synthetic dataset generator. Nothing here is imported by `app/`.
  ...
  - `encoder_training/` — Trains and evaluates encoder heads against those datasets. `python -m scripts.encoder_training generate-folds` writes all K folds' three splits; `... baselines` fits the baselines and negative controls and writes the evaluation report. Everything except the model-fitting bodies is stdlib only, so CI covers it with no ML wheels; `baselines` needs `requirements-ml.txt`. Tested by `tests/test_encoder_training_{dataset,metrics,baselines}.py`.
- `requirements-ml.txt` — Offline encoder-training dependencies (scikit-learn now, torch/transformers later). Never installed in production or in CI, and carries the Blackwell/cu128 warning for the target GPU.
