...
- `scripts/` — Offline tooling: one-time management commands, plus the synthetic dataset generator. Nothing here is imported by `app/`.
  ...
  - `encoder_training/` — Trains and evaluates encoder heads against those datasets. `generate-folds` writes all K folds' three splits; `baselines` fits the baselines and negative controls; `smoke-cuda` runs a real kernel on the training device (no model, no network — run it first on a new machine); `smoke` adds the encoder download; `probe` is Arm A, the frozen probe; `finetune` is Arm B, every layer unfrozen, and it also scores the real-text holdout in `data/realistic/` after each fold's margin and test split (`--no-holdout` skips it). Everything except the model-fitting bodies is stdlib only, so CI covers it with no ML wheels; the arms and `baselines` need `requirements-ml.txt`. Tested by `tests/test_encoder_training_{dataset,metrics,baselines,arm_a,arm_b,holdout}.py`.
