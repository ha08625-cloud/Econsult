...
* **Note on expanding past one signal:** libraries exist for six of the seven signals and **all six have now been trained** (2026-08-16), one Arm B head each at `roberta-base`. That is six separate single-signal heads, *not* one model answering six questions. The **merge step exists** (`scripts/encoder_training/merge.py`, `merge-folds`): it concatenates the six per-signal fold trees into one that `load_folds` reads unchanged, keeping every head masked where it had no label and every example's original id in `meta.source_ids`. The **joint training path now exists too** (`finetune --dataset <merged tree> --signals <heads>`, `arch_encoder_training.md` section 4b): several heads share one encoder, DD6 selects one epoch by the unweighted mean of every head's own validation macro-F1, and each head's margin is chosen independently. A single-signal call through the same path is proved bitwise-identical to the pre-existing single-signal path by test, not assumed. The **three-arm report shape now exists too** (`joint-compare`, `arch_encoder_training.md` section 4c): three fold trees in one invocation — A1 single-signal, A2 the same clusters at ~4.5x the recombinations, A3 joint — one report per signal holding all three, with A1 vs A3 as the paired comparison and A2 recorded as an untestable pair rather than silently missing. **The sweep has now run** (2026-08-17, `<signal>.joint_comparison.json`, write-up `reports/encoder_training/2026-08-17-plain-english.md`): joint training beats the single-signal heads on four of six signals on the synthetic test set, and makes the real-text `null → true` cell several times worse on every one of them. See `arch_training.md` section 10 — the finding is about the *data* (no head is ever taught that dense clinical language about another symptom is still `null` for it), and it makes the multi-symptom recombinations of 12.5 the critical path rather than an option. Results table in `arch_training.md` section 10; write-up in `reports/encoder_training/2026-08-16-plain-english.md`. The plan of record is `planned_updates/joint_multi_head_training_implementation.md` (supersedes `multi_symptom_training_expansion.md`), summarised in `arch_training.md` 12.8.
...
* **Note on `null → true`:** the cell that invents a symptom into a patient's pre-filled form. On the synthetic test set it runs 1.34% (`fever`) to 4.04% (`nocturia`) across the six single-signal heads, and the joint six-head model improves it on four of six. **On the 67 real submissions the same cell runs 47%–89% for the joint model and 2%–53% for the single-signal ones** (2026-08-17 sweep). The joint model scores 39.1% across the 402 real answers against 66.7% for replying `null` to everything — i.e. worse than doing nothing on real text, while looking better on recombinations. Read `arch_training.md` section 10 before quoting any synthetic figure here. No head has been through a safety review against `arch_encoder.md`'s boundary, and none is wired to anything.
