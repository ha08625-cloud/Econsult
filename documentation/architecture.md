...
* **Invariants:**
  ...
  * `step_labels` in `runs.json` is presentational and never reaches an argv; it exists so a 27-step run reads as a checklist. `catalogue.py` validates only that there is one non-empty label per step, because a list out of step with the steps would label the wrong rows.
