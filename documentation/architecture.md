...
* **Note on expanding past one signal:** libraries exist for six of the seven signals and all six now generate five-fold datasets at fever's recipe, but nothing but `fever_present` has been *trained* — the six fine-tunes are an unspent GPU hour, and `reports/encoder_training/` holds fever numbers only. The plan of record is `planned_updates/multi_symptom_training_expansion.md`, summarised in `arch_training.md` 12.8. Six single-signal runs need no code change; joint multi-head training needs a merge step, per-head margin selection and a report shape that can hold several signals.
* **Note before comparing signals:** only the `fever_*` and `dysuria_*` confounder libraries carry cluster markers, so the other four signals' effective sample sizes are upper bounds and their intervals are narrower than the truth. Every report now prints the measured coverage and this warning above its own headline. See `arch_training.md` section 10.
