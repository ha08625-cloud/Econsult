...
* **Note on expanding past one signal:** libraries exist for six of the seven signals and **all six have now been trained** (2026-08-16), one Arm B head each at `roberta-base`. That is six separate single-signal heads, *not* one model answering six questions — joint multi-head training still needs a merge step, per-head margin selection and a report shape that can hold several signals. Results table in `arch_training.md` section 10; write-up in `reports/encoder_training/2026-08-16-plain-english.md`. The plan of record is `planned_updates/multi_symptom_training_expansion.md`, summarised in `arch_training.md` 12.8.
