...
- **Session Cookie Format Validation.** `admin_sessions.session_id` is a Postgres `uuid` column; `get_session_context` and `delete_session` both cast the raw cookie value with `%s::uuid`. `require_admin` (`app/core/admin_context.py`) and the logout endpoint (`admin_auth_router.py`) validate the cookie is a well-formed UUID in Python (`is_valid_session_id`) before it ever reaches the repository, so a corrupted or hand-crafted cookie value returns a normal 401 (or a clean no-op on logout) instead of a psycopg2 `InvalidTextRepresentation` 500.
...
- **Request Body Size Pre-Check (DoS Hardening).** `POST /form/finish` is mounted on `MaxBodySizeRoute` (`app/core/max_body_size_route.py`), which checks `Content-Length` against `MAX_FINISH_REQUEST_BYTES` (`MAX_TOTAL_SIZE_BYTES` + a 2 MiB multipart/JSON-payload overhead margin) in the route wrapper that runs *before* FastAPI resolves `photos: list[UploadFile]` — rejecting oversized requests with 413 before any multipart parsing occurs, not after. Inside `form_finish`, the photo-count check (`MAX_FILE_COUNT`) also runs before any file bytes are read into memory, so an over-count submission is rejected without first buffering every file. Neither FastAPI nor Starlette imposes a body size cap by default, and the 30/minute rate limit only bounds repetition, not the size of a single request — this is the control for that gap. Caveat: the Content-Length check cannot catch a request that omits the header (chunked transfer-encoding) or lies about it; the in-handler size checks (which measure actual read bytes) remain the backstop for that case.
- **Server-Side Length Limits on Patient Text.** `request_validation.py` enforces a max length on every free-text field accepted by the unauthenticated `/form/*` endpoints — `free_text` and `additional_text` (5000 chars), names (`first_name`, `last_name`, `preferred_name`, `submitter_name`, `submitter_relationship`, 255 chars), and `contact_preferences` free text (`email_address` 255, `phone_number` 100, `usual_doctor_name` 255, `best_time_to_call` 255). `_validate_optional_str`/`_validate_required_str` also reject non-string values (e.g. a nested JSON object) before they can reach `pdf_formatter.py`, where a non-string would otherwise raise a `TypeError` inside the PDF worker instead of failing the request cleanly with a 422. This bounds both the per-request payload and the append-only `RuntimeState` history (every `/form/update` call re-stores the full state, so an unbounded field is also a storage-growth and PDF-rendering-time vector) — closing the gap left by the `Content-Length` pre-check above, which only bounds `/form/finish`.
